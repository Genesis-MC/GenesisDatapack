...
execute unless data entity @s Inventory[{Slot:-106b}].tag.gen.ability run return 0
# check cooldown
execute store result score #cooldown gen.ability run data get entity @s Inventory[{Slot:-106b}].tag.gen.ability.cooldown
execute if score #cooldown gen.ability > .gametime gen.time run tellraw @a[tag=gen.dev.debug] [{"translate":"Offhand ability %s on cooldown until: %s","color":"#aaffaa","with":[{"nbt":"Inventory[{Slot:-106b}].tag.gen.ability.function","entity":"@s","color":"#22cc22"},{"score":{"objective":"gen.ability","name":"#cooldown"},"color":"#22cc22"}]}]
...
execute at @s run function gen:core/internal/run_function with entity @s Inventory[{Slot:-106b}].tag.gen.ability

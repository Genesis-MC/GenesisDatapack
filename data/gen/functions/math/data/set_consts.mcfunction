scoreboard players set #-1 gen.const -1
scoreboard players set #2 gen.const 2
scoreboard players set #4 gen.const 4
scoreboard players set #8 gen.const 8
scoreboard players set #9 gen.const 9
scoreboard players set #10 gen.const 10
scoreboard players set #72 gen.const 72
scoreboard players set #100 gen.const 100
scoreboard players set #363 gen.const 363
scoreboard players set #1000 gen.const 1000
scoreboard players set #4000 gen.const 4000
scoreboard players set #20000 gen.const 20000

# int limits cuz i alwaus forget them
# -2147483648..2147483647
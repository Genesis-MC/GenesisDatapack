...
execute align xyz run summon armor_stand ~.5 ~-1 ~.5 {Tags:["gen.professions.mineralogy.crystalarium","normCrystalarium","Tier1"],NoGravity:1b,Invulnerable:1b,Invisible:1b,ShowArms:1b,CustomNameVisible:0b,CustomName:'{"translate":"Tier 1","bold":true,"italic":false}',DisabledSlots:4079422}
execute positioned ~ ~-1 ~ run scoreboard players set @e[distance=..0.9,sort=nearest,limit=1,tag=gen.professions.mineralogy.crystalarium] gen.profession.mineralogy.growth_level 0
execute positioned ~ ~-1 ~ run scoreboard players set @e[distance=..0.9,sort=nearest,limit=1,tag=gen.professions.mineralogy.crystalarium] gen.profession.mineralogy.growth_req 24